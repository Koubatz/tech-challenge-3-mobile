  ...
  const handleHealthCheck = async () => {
    ...
    try {
      ...
      if (data.success && data.docId) {
        ...
        Alert.alert(
          "Success",
          `Timestamp saved to Firestore with ID: ${data.docId}`
        );
      } else {
        console.error("Firebase health check failed:", data);
        Alert.alert(
          "Error",
          `Failed to save timestamp: ${data.error || JSON.stringify(data)}`
        );
      }
    } catch (error) {
      console.error(
      ...
      Alert.alert(
        "Error",
        "Failed to connect to Firebase backend. See console for details."
      );
    } finally {
      setIsLoading(false);
    }
